        ...
        <Container>
            ...
            <Grid container xs="12">
                <Grid xs="4">
                    ...
                    {post.tags.map(t => <Badge key={t.id} bg="info">{t.name} TAG</Badge>)}
                </Grid>
                <Grid xs="8">
                    <PostContent/>
                </Grid>
...
const CommentForm = ({ postId, comments, setComments }) => {
    ...
    const addComment = async (event) => {
        ...
        //Them comment http://127.0.0.1:8000/posts/{postId}/add-comment/
        const res = await authAxios().post(endpoints['add-comments'](postId), {
            'content': content,
            'post': postId,
            'user': user.id
        });
