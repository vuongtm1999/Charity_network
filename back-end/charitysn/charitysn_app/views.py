...
class PostViewSet(viewsets.ViewSet,
    ...
    def get_comments(self, request, pk):
        # lay tat ca comment cua bai post co {id} dc truyen vao
...
class UserViewSet(viewsets.ViewSet,
    ...
    # Lay tat ca bai post ma nguoi dung da like
    @action(methods=['get'], detail=True, url_path='liked')
    def get_liked(self, request, pk):
        # lay tat ca comment cua user co {id} dc truyen vao
        liked = self.get_object().user_likes.all()

        return Response(LikePostSerializer(liked, many=True).data,
                        status=status.HTTP_200_OK)
